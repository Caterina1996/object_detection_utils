...
class Evaluator:
    ...
    def GetPascalVOCMetrics(self,boundingboxes,conf_thr=0.7,IOUThreshold=0.5,method=MethodAveragePrecision.EveryPointInterpolation,showGraphic=True):
        ...
        count_gt2 = np.zeros(n_classes, dtype=int)
        count_dets = np.zeros(n_classes, dtype=int)
        conf_m = np.zeros([n_classes, n_classes + 2], dtype=int)
        ...
        sum_conf_m_0 = conf_m.sum(axis=0)
        sum_conf_m_1 = conf_m.sum(axis=1)

        for c in range(n_classes):
            conf_m[c, n_classes] = count_gt[c] - sum_conf_m_1[c]

        for c in range(n_classes):
            conf_m[c, n_classes+1] = count_dets[c] - sum_conf_m_0[c]
