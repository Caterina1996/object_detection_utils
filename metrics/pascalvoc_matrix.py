...
matrix, classes = evaluator.GetPascalVOCMetrics(allBoundingBoxes, confThreshold, iouThreshold, MethodAveragePrecision.EveryPointInterpolation, showPlot)
print(matrix)
print(classes)
...
df = pd.DataFrame.from_records(matrix, index=allClasses)
df.to_csv(os.path.join(savePath, name + '.csv'), header = header)
