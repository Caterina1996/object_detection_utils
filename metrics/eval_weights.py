...
for n, id_ in enumerate(pred_list):
...
for n, id_ in enumerate(gt_list):
    path = os.path.join(gt_im_path, id_)
    img = imread(path,as_gray = True)
    img = resize(img, (IMG_HEIGHT, IMG_WIDTH), mode='constant', preserve_range=True)
    gts[n] = img
...
data = {'Run': [run_name], 'acc': [acc], 'prec': [prec], 'rec': [rec], 'fall': [fall], 'f1': [f1]}

df = pd.DataFrame(data)
print(df)

df.to_excel(os.path.join(save_path,'metrics_weighted.xlsx'))
